...
test_that("pdf_underlay_stamp works", {
  pdf_file <- 'pdf-example-password.original.pdf'
  stamp_file <- pdf_create_test_stamp()
  outfile_all <- tempfile(fileext = ".pdf")
  outfile_one <- tempfile(fileext = ".pdf")
  bg_file <- tempfile(fileext = ".pdf")
  outfile_bg <- tempfile(fileext = ".pdf")
  on.exit(unlink(c(stamp_file, outfile_all, outfile_one, bg_file, outfile_bg)))

  # pdf_underlay_stamp should stamp all pages by default
  pdf_underlay_stamp(
    pdf_file,
    stamp_file,
    output = outfile_all,
  ...
  # pdf_underlay_stamp only stamps selected pages
  pdf_underlay_stamp(
    pdf_file,
    stamp_file,
    output = outfile_one,
  ...
  # Test if we can underlay new backgrounds
  grDevices::pdf(bg_file, bg = "#00008080")
  grid::grid.newpage()
  ...
  expect_equal(pdf_length(outfile_bg, password = "test"), 1L)

  # pdf_underlay_stamp errors on out-of-range pages
  expect_error(pdf_underlay_stamp(pdf_file, stamp_file, pages = 999, password = "test"))
})
